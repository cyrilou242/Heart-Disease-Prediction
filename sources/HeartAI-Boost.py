...
def main(data_string,filename="processed.all.data"):
    ...
    model_savename = filename + 'ModelBoost.p'
    ...
    r1 = "%-30s%-4.2f%-1s" % ('La chance d\'un accident cardiaque est de ', 100 * prediction[0][1], '%')
...
def faireModele(filename="processed.all.data"):
    ...
    model = ensemble.GradientBoostingClassifier.fit(training_scaled,data['y'])
...
def crossValidation(data, verbose=0, shuffling=True):
    ...
    for train_index, test_index in kf.split(data['X']):
        ...
        clf = model_selection.GridSearchCV(
            ...
            {'loss': ['deviance'], 'n_estimators': [250], 'max_depth' : [2]
             } #FIXME : TRYME : mettre des parametres, l'opti est peut être pas ouf
        ).fit(X_train, y_train)
...
#FIXME petit probleme à l'execution unitaire on dirait, a revoir
#s = main("62.0 0.0 4.0 140.0 268.0 0.0 2.0 160.0 0.0 3.6 3.0 2.0 3.0")
#print('lol')
#print(s)
